   ...
   public class wwp_calendar_editevent : GXProcedure
   {
      ...
      protected override void ExecutePrivate( )
      {
         ...
         if ( AV22Trn_AgendCalendar.Success() )
         {
            ...
            AV33EventDescription = AV20Title + " - " + context.localUtil.Format( AV17EventStartDate, "99/99/99 99:99") + " to " + context.localUtil.Format( AV15EventEndDate, "99/99/99 99:99");
            ...
            {
               new prc_sendresidentnotification(context ).execute(  "New Calendar Event",  AV33EventDescription,  "AGENDA",  AV36Metadata,  AV29AddressGroup) ;
               new GeneXus.Programs.wwpbaseobjects.notifications.common.wwp_sendnotification(context ).execute(  "AgendaNotification",  "AgendaEvents",  "",  "",  "New Agenda Created",  AV33EventDescription,  AV33EventDescription,  formatLink("wp_calendaragenda.aspx") ,  "",  "",  true) ;
            }
            else if ( StringUtil.StrCmp(Gx_mode, "UPD") == 0 )
            {
               new prc_sendresidentnotification(context ).execute(  "Calendar Event Updated",  AV33EventDescription,  "AGENDA",  AV36Metadata,  AV29AddressGroup) ;
               new GeneXus.Programs.wwpbaseobjects.notifications.common.wwp_sendnotification(context ).execute(  "AgendaNotification",  "AgendaEvents",  "",  "",  "Agenda Event Updated",  AV33EventDescription,  AV33EventDescription,  formatLink("wp_calendaragenda.aspx") ,  "",  "",  true) ;
            }
         }
         else
         {
