		...
		this.Start = function() {
			   ...
			   try {
					...
					const themes = this.BC_Trn_ThemeCollection.map(theme => {
						...
						// Convert and sort ThemeCtaColors by CtaColorName
						if (theme.CtaColor) {
							// Define mapping from number to word
							const numberToWord = {
								'1': 'One',
								'2': 'Two',
								'3': 'Three',
							...
							res.ThemeCtaColors = theme.CtaColor
							.map(ctaColor => {
								// Extract the number from CtaColorName (e.g., "CtaColor1" -> "1")
								const numberMatch = ctaColor.CtaColorName.match(/ctaColor(\d+)/);
								const number = numberMatch ? numberMatch[1] : null;
								
								// Create a new object with renamed property
								return {
									...ctaColor,
									CtaColorName: number ? `CtaColor${numberToWord[number]}` : ctaColor.CtaColorName
								};
							})
							.sort((a, b) => {
								// Sort based on word ordering
								const wordOrder = ['One', 'Two', 'Three', 'Four', 'Five', 'Six'];
								const aWord = a.CtaColorName.replace('CtaColor', '');
								const bWord = b.CtaColorName.replace('CtaColor', '');
								return wordOrder.indexOf(aWord) - wordOrder.indexOf(bWord);
							});
						}
