   ...
   public class wwp_selectimportfile : GXWebComponent
   {
      ...
      protected void RenderHtmlOpenForm( )
      {
         ...
         if ( StringUtil.Len( sPrefix) == 0 )
         {
            ...
            context.skipLines(1);
            context.WriteHtmlTextNl( "<form id=\"MAINFORM\" autocomplete=\"off\" name=\"MAINFORM\" method=\"post\" tabindex=-1  class=\"form-horizontal FormNoBackgroundColor\" data-gx-class=\"form-horizontal FormNoBackgroundColor\" novalidate action=\""+formatLink("wwpbaseobjects.wwp_selectimportfile.aspx", new object[] {UrlEncode(StringUtil.RTrim(AV16TransactionName)),UrlEncode(StringUtil.RTrim(AV11ImportType)),UrlEncode(StringUtil.RTrim(AV7ExtraParmsJson))}, new string[] {"TransactionName","ImportType","ExtraParmsJson"}) +"\">") ;
      ...
      protected void E131Y2( )
      {
         ...
         if ( ! String.IsNullOrEmpty(StringUtil.RTrim( AV9FilterToUpload)) )
         {
            if ( ( StringUtil.StrCmp(StringUtil.Upper( AV10FilterToUploadExt), "JSON") == 0 ) && ( StringUtil.StrCmp(AV11ImportType, "JSON") == 0 ) )
            {
               ...
               {
                  AV12LastErrorType = 2;
                  GX_msglist.addItem(new WorkWithPlus.workwithplus_web.dvmessagegetbasicnotificationmsg(context).executeUdp(  "Form imported successfully",  "The dynamic form was imported successfully.",  "success",  "",  "na",  ""));
            ...
            else
            {
               if ( ( ( StringUtil.StrCmp(StringUtil.Upper( AV10FilterToUploadExt), "CSV") == 0 ) && ( StringUtil.StrCmp(AV11ImportType, "CSV") == 0 ) ) || ( ( StringUtil.StrCmp(StringUtil.Upper( AV10FilterToUploadExt), "XLSX") == 0 ) && ( StringUtil.StrCmp(AV11ImportType, "Excel") == 0 ) ) || ( ( StringUtil.StrCmp(StringUtil.Upper( AV10FilterToUploadExt), "JSON") == 0 ) && ( StringUtil.StrCmp(AV11ImportType, "JSON") == 0 ) ) )
               {
                  ...
                  if ( new GeneXus.Programs.wwpbaseobjects.wwp_importdata(context).executeUdp(  AV16TransactionName,  AV11ImportType,  AV22BlobPath,  AV7ExtraParmsJson, out  AV6ErrorMsgs) )
                  {
                     ...
                     GX_msglist.addItem(new WorkWithPlus.workwithplus_web.dvmessagegetbasicnotificationmsg(context).executeUdp(  "File import success",  AV14ResultMsg,  "success",  "",  "na",  ""));
                     this.executeExternalObjectMethod(sPrefix, false, "WWPActions", "WCPopup_Close", new Object[] {(string)AV14ResultMsg}, false);
                  }
                  else
                  {
               ...
               else
               {
                  ...
                  AssignAttri(sPrefix, false, "AV14ResultMsg", AV14ResultMsg);
                  GX_msglist.addItem(new WorkWithPlus.workwithplus_web.dvmessagegetbasicnotificationmsg(context).executeUdp(  "Error importing file",  AV14ResultMsg,  "error",  "",  "na",  ""));
               }
            }
         }
         else
      ...
      protected void S122( )
      {
         ...
         GX_msglist.addItem(new WorkWithPlus.workwithplus_web.dvmessagegetbasicnotificationmsg(context).executeUdp(  "Error importing file",  AV14ResultMsg,  "error",  "",  "false",  ""));
      }
      ...
      protected void define_styles( )
      {
         ...
         {
            context.AddJavascriptSource(StringUtil.RTrim( ((string)Form.Jscriptsrc.Item(idxLst))), "?202542411435383", true, true);
            idxLst = (int)(idxLst+1);
         }
         if ( ! outputEnabled )
         {
      ...
      {
         context.AddJavascriptSource("wwpbaseobjects/wwp_selectimportfile.js", "?202542411435383", false, true);
